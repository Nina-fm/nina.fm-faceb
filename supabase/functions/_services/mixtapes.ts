import { mixtapeAuthorsWithAuthorsRelation, mixtapeTagsWithTagsRelation, tracksRelation } from '../_shared/relations.ts'
import { formatAuthorNames, handleLocalFileUrl } from '../_shared/utils.ts'
import { MixtapeExt, MixtapeParams, MixtapeParamsExt } from '../_types/mixtapes.ts'
...
export class MixtapesService extends Service {
  ...
  format(mixtape: MixtapeExt) {
    ...
    const authors = mixtapes_authors
      ? mixtapes_authors.map(({ author, position }) => {
      ...
      : []
    const authors_text = formatAuthorNames(authors)
    const cover_url = cover
      ? handleLocalFileUrl(this.supabase.storage.from('covers').getPublicUrl(cover).data.publicUrl)
      : null
    const tags = mixtapes_tags ? mixtapes_tags.map(({ tag }) => ({ ...tag })) : []
  ...
  async findAll() {
    const { data: mixtapes, error } = await this.supabase
      .from('mixtapes')
      .select(`*, ${mixtapeAuthorsWithAuthorsRelation}, ${mixtapeTagsWithTagsRelation}, ${tracksRelation}`)
  ...
  async findByName(name: string) {
    const { data: mixtapes, error } = await this.supabase.from('mixtapes').select('*').ilike('name', `%${name}%`)
  ...
  async updateAuthors(id: string | number, authors: AuthorParamsExt[] = []) {
    const _authors = new AuthorsService(this.headers)
    const authorIds = await _authors.getIdsOrCreate(authors)
    const { data: existing } = await this.supabase.from('mixtapes_authors').select('*').match({ mixtape_id: id })

    const toDeleteIds = existing?.length
      ? existing.filter((ma) => !authorIds.includes(ma.author_id)).reduce((res, ma) => [...res, ma.author_id], [])
      : []
  ...
  async updateTags(id: string | number, tags: TagParams[] = []) {
    const _tags = new TagsService(this.headers)
    const tagIds = await _tags.getIdsOrCreate(tags)
    const { data: existing } = await this.supabase.from('mixtapes_tags').select('*').match({ mixtape_id: id })

    const toDeleteIds = existing?.length
      ? existing.filter((ma) => !tagIds.includes(ma.tag_id)).reduce((res, ma) => [...res, ma.tag_id], [])
      : []
  ...
  async create(mixtapeData: MixtapeParams) {
    const { data: mixtape, error } = await this.supabase.from('mixtapes').insert([mixtapeData]).select().single()
  ...
  async delete(id: string | number) {
    const { error } = await this.supabase.from('mixtapes').delete().match({ id })
