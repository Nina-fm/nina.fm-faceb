...
export class Service {
  ...
  async fileExists(file: string, bucket: string) {
    ...
    const { data } = await this.supabase.storage.from(bucket).list(folder, { search })
    return !!data?.length
  }
  ...
  async handleFile(fileModel: FileModel, bucket: string) {
    ...
    // Upload the File
    const { data: coverFile, error: coverError } = await this.supabase.storage.from(bucket).upload(filename, file)
