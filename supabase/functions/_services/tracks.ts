...
export class TracksService extends Service {
  ...
  async findByMixtape(mixtapeId: string | number) {
    const { data: tracks, error } = await this.supabase.from("tracks").select("*").eq("mixtape_id", mixtapeId)
   ...
   * Create new tracks
   */
  async createForMixtape(mixtapeId: string | number, tracksData: TrackParams[]) {
   ...
   * Update tracks
   */
  async updateForMixtape(mixtapeId: string | number, tracksData: TrackParams[]) {
    const tracksDataIds = tracksData.reduce((r, t) => [...r, ...(t.id ? [t.id] : [])], [] as (string | number)[])
