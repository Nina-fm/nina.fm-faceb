...
export const useSnackbarStore = defineStore("snackbar", () => {
  ...
  const snackInfo = (message: string) =>
    enqueueSnackbar({ color: "info", message });

  const snackSuccess = (message: string) =>
    enqueueSnackbar({ color: "success", message });

  const snackWarning = (message: string) =>
    enqueueSnackbar({ color: "warning", message });

  const snackError = (message: string) =>
    enqueueSnackbar({ color: "error", message });
